  ...
  constructor(private prisma: PrismaService) {
    this.server = new Server({
      ...
      onLoadDocument: async ({ documentName }) => {
        ...
        if (!record) throw new Error(`${room.handler.label} ${room.id} not found`)
        return loadDoc(new Uint8Array(record.data), await room.handler.listUpdates(this.prisma, room.id))
      },
      onStoreDocument: async ({ document, documentName }) => {
        const room = resolveRoom(documentName)
        await room.handler.storeSnapshot(this.prisma, room.id, Y.encodeStateAsUpdate(document))
      },
