...
export const parseChartRoomName = (value: string) => (value.startsWith(prefix) ? value.slice(prefix.length) : null)

export const findChartRecord = (prisma: PrismaService, id: string) => prisma.chart.findUnique({ where: { id } })
...
export const appendChartUpdate = (prisma: PrismaService, id: string, data: Uint8Array) =>
...
export const storeChartSnapshot = (prisma: PrismaService, id: string, data: Uint8Array) =>
  prisma.chart.update({
    where: { id },
