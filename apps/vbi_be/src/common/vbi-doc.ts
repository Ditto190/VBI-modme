...
export const buildChartDSL = (
  snapshot?: Uint8Array,
  updates: Uint8Array[] = [],
): VBIChartDSL => new VBIChartBuilder(createDoc(snapshot, updates)).build();

export const buildInsightDSL = (
  snapshot?: Uint8Array,
  updates: Uint8Array[] = [],
): VBIInsightDSL => new VBIInsightBuilder(createDoc(snapshot, updates)).build();

export const buildReportDSL = (
  snapshot?: Uint8Array,
  updates: Uint8Array[] = [],
): VBIReportDSL => new VBIReportBuilder(createDoc(snapshot, updates)).build();
