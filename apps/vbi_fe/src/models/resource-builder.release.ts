import type { BuilderSession, BuilderStoreState } from './resource-builder.types'

const RELEASE_DELAY_MS = 0

type GetState<TBuilder> = () => BuilderStoreState<TBuilder>
type SetState<TBuilder> = (
  updater: (state: BuilderStoreState<TBuilder>) => Partial<BuilderStoreState<TBuilder>>,
) => void

const removeSession = <TBuilder>(sessions: Record<string, BuilderSession<TBuilder>>, resourceId: string) => {
  const next = { ...sessions }
  delete next[resourceId]
  return next
}

export const createReleaseController = <TBuilder>(getState: GetState<TBuilder>, setState: SetState<TBuilder>) => {
