...
export const ReportWorkspace = memo(() => {
  ...
  const viewPages = useMemo(
    () =>
      pages.map((page) => ({
        chartBuilder:
          page.chartId && page.chartId in chartBuilders
            ? (chartBuilders[page.chartId] ?? null)
            : null,
        hasChart: !!page.chartId,
        hasInsight: !!page.insightId,
        insightBuilder:
          page.insightId && page.insightId in insightBuilders
            ? (insightBuilders[page.insightId] ?? null)
            : null,
        page,
      })),
