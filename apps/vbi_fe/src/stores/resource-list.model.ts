...
export const setResourceSearchText = (
  state: ResourceListState,
  searchText: string,
) => ({
...
export const selectFilteredResourceIds = (state: ResourceListState) => ({
  selectedRowKeys: getFilteredItems(state.items, state.searchText).map(
    (item) => item.id,
  ),
});
...
export const createResourceListActions = <TState extends ResourceListState>(
  ...
  selectAllFiltered: () =>
    set((state) => selectFilteredResourceIds(state) as Partial<TState>),
  setSearchText: (searchText) =>
    set((state) => setResourceSearchText(state, searchText) as Partial<TState>),
  setSelectedRowKeys: (selectedRowKeys) =>
    set({ selectedRowKeys } as Partial<TState>),
});

export const loadResourceItems = async <
  TState extends LoadableResourceListState,
>(
  set: SetState<TState>,
