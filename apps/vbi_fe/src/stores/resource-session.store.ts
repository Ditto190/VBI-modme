...
import type {
  BuilderByKind,
  BuilderModel,
} from '../models/resource-builder.types';
...
export const connectResourceSession = async (
  kind: ResourceKind,
  resourceId: string,
  userName: string,
) => {
  ...
  setCollaborativeUser(
    model.getState().sessions[resourceId]?.provider,
    userName,
  );
};

export const releaseResourceSession = async (
  kind: ResourceKind,
  resourceId: string,
) => {
  if (!resourceId) return;
