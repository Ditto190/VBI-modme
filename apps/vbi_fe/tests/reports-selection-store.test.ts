...
describe('reports store selection', () => {
  ...
  test('selectAllFiltered selects matching reports only', () => {
    useReportsStore.setState({
      items: [createReportItem('report-1', 'Sales'), createReportItem('report-2', 'Marketing')],
      searchText: 'sale',
    })
  ...
  test('deleteSelected removes selected reports and clears selection', async () => {
    ...
    expect(resourceApi.removeResource).toHaveBeenCalledWith('report', 'report-1')
    expect(resourceApi.removeResource).toHaveBeenCalledWith('report', 'report-2')
