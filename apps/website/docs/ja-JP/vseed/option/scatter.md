...
`xAxis`  : X軸チャネル。`複数の指標`をサポートし、指標値をX軸にマッピングします

`yAxis`  : Y軸チャネル。`複数の指標`をサポートし、指標値をY軸にマッピングします

`color`  : 色チャネル。`複数の次元`または`1つの指標`をサポートします。次元色は異なるデータ系列の区別に、指標色は指標値を図形色へ線形マッピングするために使用されます
...
[
  ...
  {
    id: 'sales', alias: '売上', encoding: 'yAxis'
  }
]
