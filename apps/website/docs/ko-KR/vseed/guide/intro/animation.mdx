...
#### 2.1 highlight(그룹 강조) + atmo(분위기 효과)
...
레이더 차트 루프 애니메이션은 `atmo.effect`를 통해 지속적인 분위기 움직임으로 표현됩니다.

#### 2.1 none(루프) + atmo(ripple)
...
산점도 루프 애니메이션은 일반적으로 지속적인 리듬을 위해 `atmo`로 표현됩니다.
