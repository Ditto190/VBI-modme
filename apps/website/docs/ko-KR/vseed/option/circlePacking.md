...
:::info{title=인코딩 매핑}
원형 패킹 차트는 다음 시각 채널을 지원합니다:

`color`: 색상 채널, `여러 차원` 또는 `하나의 지표`를 지원합니다
...
:::note{title=설명}
원형 패킹 차트는 계층 데이터를 표시하며, 원의 크기로 수치 크기를 나타냅니다.
...
\- 원 크기를 매핑할 수치 필드가 최소 1개 필요합니다

\- 계층 분할에 사용할 차원 필드가 최소 1개 필요합니다
