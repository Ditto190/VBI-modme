...
`angle`  : 각도 채널, `여러 지표`를 지원하며 지표 값을 부채꼴 각도에 매핑합니다

`detail` : 세부 채널, `여러 차원`을 지원하며 같은 색상 계열 안에서 더 세분화된 데이터를 표시할 때 사용합니다

`color`  : 색상 채널, `여러 차원` 또는 `하나의 지표`를 지원합니다. 차원 색상은 서로 다른 데이터 시리즈를 구분하고, 지표 색상은 지표 값을 그래픽 색상에 선형 매핑하는 데 사용합니다

`tooltip`: 툴팁 채널, `여러 차원`과 `여러 지표`를 지원하며 데이터 포인트에 마우스를 올렸을 때 표시됩니다
...
- 전체 데이터와 각 부분의 비율을 동시에 표시해야 하는 경우

- 전체와 부분의 관계를 강조

- 중앙 영역에 핵심 지표나 제목을 표시해야 하는 경우
:::
...
- 범례, 데이터 레이블, 툴팁, 비율 계산, 중앙 텍스트가 기본으로 활성화됩니다
:::
