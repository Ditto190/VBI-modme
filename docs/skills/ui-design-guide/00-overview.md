...
| [17-demo-connector.md](./17-demo-connector.md)                     | Connector/bootstrap 模块完整代码（demoConnector/localConnector）                               |
...
| hooks（standard/minimalist/professional） | `src/hooks/`（当前 practice）                         |
| hooks（vbi-react-starter）                | `@visactor/vbi-react`                                 |
| VBI 核心 API（VBI / VBIChartBuilder 等）  | `@visactor/vbi` 主入口（实践中常配合 bootstrap 模块） |
...
| localConnector（本地数据） | `practices/professional/src/utils/localConnector.ts`           |
| VSeedRender                | `practices/professional/src/components/Render/VSeedRender.tsx` |
| 完整 UI 示例               | `practices/standard/src/App/`                                  |
