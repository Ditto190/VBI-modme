import { beforeEach, describe, expect, rs, test } from '@rstest/core'
import {
  chatMethod,
  createDeepSeek,
  createStream,
  deepseekModel,
  jsonSchema,
  resetDeepSeekMocks,
  streamText,
} from './deepseek-provider.mock.js'

rs.mock('@ai-sdk/deepseek', () => ({ createDeepSeek }))
rs.mock('ai', () => ({ jsonSchema: rs.fn(), streamText }))

describe('createDeepSeekModelProvider', () => {
  beforeEach(resetDeepSeekMocks)

  test('streams text through a DeepSeek model', async () => {
    streamText.mockReturnValue({
      fullStream: createStream(
        { text: 'reasoning', type: 'reasoning-delta' },
        { text: 'final ', type: 'text-delta' },
        { text: 'answer', type: 'text-delta' },
      ),
    })
    const { createDeepSeekModelProvider } = await import('../src/model/deepseek-provider.js')
    const provider = createDeepSeekModelProvider({
      apiKey: 'test-key',
      baseUrl: 'https://api.deepseek.com',
      model: 'deepseek-reasoner',
    })
    const chunks: string[] = []
    const result = await provider.streamTurn({
      handlers: { onTextDelta: (chunk) => void chunks.push(chunk) },
      history: [{ content: 'hello', role: 'user' }],
      tools: {},
    })
    expect(createDeepSeek).toHaveBeenCalledWith({ apiKey: 'test-key', baseURL: 'https://api.deepseek.com' })
    expect(chatMethod).toHaveBeenCalledWith('deepseek-reasoner')
    expect(streamText).toHaveBeenCalledWith(
      expect.objectContaining({
        messages: [{ content: 'hello', role: 'user' }],
        model: deepseekModel,
        providerOptions: { deepseek: { thinking: { type: 'disabled' } } },
        tools: {},
      }),
    )
    expect(chunks).toEqual(['final ', 'answer'])
    const reasoningPart = (result.assistant.content as Array<{ type: string; text: string }>).find(
      (p) => p.type === 'reasoning',
    )
    expect(reasoningPart?.text).toBe('reasoning')
    expect(result.outcome).toEqual({ content: 'final answer', type: 'final' })
  })

  test('preserves reasoning content in multi-turn history for deepseek reasoner', async () => {
    streamText.mockReturnValue({ fullStream: createStream({ text: 'done', type: 'text-delta' }) })
    const { createDeepSeekModelProvider } = await import('../src/model/deepseek-provider.js')
    const provider = createDeepSeekModelProvider({
      apiKey: 'test-key',
      baseUrl: 'https://api.deepseek.com',
      model: 'deepseek-reasoner',
    })
    await provider.streamTurn({
      history: [
        { content: 'use tool', role: 'user' },
        {
          content: [
            { type: 'reasoning', text: 'kept reasoning' },
            { type: 'text', text: 'using tool' },
            { type: 'tool-call', toolCallId: 'call-1', toolName: 'bash', input: { command: 'pwd' } },
          ],
          role: 'assistant',
        },
        {
          content: [
            { type: 'tool-result', toolCallId: 'call-1', toolName: 'bash', output: { type: 'text', value: 'ok' } },
          ],
          role: 'tool',
        },
      ],
      tools: {},
    })
    expect(streamText).toHaveBeenCalledWith(
      expect.objectContaining({
        messages: expect.arrayContaining([
          expect.objectContaining({
            content: expect.arrayContaining([{ text: 'kept reasoning', type: 'reasoning' }]),
            role: 'assistant',
          }),
        ]),
        providerOptions: { deepseek: { thinking: { type: 'disabled' } } },
      }),
    )
  })

  test('handles tool calls correctly', async () => {
    streamText.mockReturnValue({
      fullStream: createStream({ toolCallId: 'call-1', toolName: 'bash', input: { command: 'ls' }, type: 'tool-call' }),
    })
    const { createDeepSeekModelProvider } = await import('../src/model/deepseek-provider.js')
    const provider = createDeepSeekModelProvider({
      apiKey: 'test-key',
      model: 'deepseek-reasoner',
    })
    const result = await provider.streamTurn({
      history: [{ content: 'list files', role: 'user' }],
      tools: { bash: { description: 'run bash', inputSchema: jsonSchema({ type: 'object' }), strict: true } },
    })
    expect(result.assistant.role).toBe('assistant')
    const toolCalls = (result.assistant.content as Array<{ type: string }>).filter((p) => p.type === 'tool-call')
    expect(toolCalls).toHaveLength(1)
    expect((toolCalls[0] as unknown as { toolName: string }).toolName).toBe('bash')
    expect(result.outcome.type).toBe('tool')
  })
})
