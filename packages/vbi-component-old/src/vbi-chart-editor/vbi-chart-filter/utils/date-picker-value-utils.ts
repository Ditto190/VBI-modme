...
export function formatDateInput(value: VBIWhereDateInput | null | undefined): string {
  if (!value) return ''
...
export function toPickerDate(value: VBIWhereDateInput | null | undefined): Dayjs | null {
...
export function fromPeriodPickerDate(unit: VBIWhereDatePeriod['unit'], value: Dayjs): VBIWhereDatePeriod {
  switch (unit) {
    case 'year':
      return { unit, year: value.year() }
    case 'quarter':
      return { unit, year: value.year(), quarter: quarterFromMonth(value.month()) }
    case 'month':
...
function getISOWeekParts(date: Date): { year: number; week: number } {
  ...
  return { year: value.getUTCFullYear(), week: Math.ceil((diff / 86400000 + 1) / 7) }
}
