...
import { isDateSchemaType } from 'src/utils/data/fieldRole'
...
export type DimensionDateAggregateKey = (typeof DIMENSION_DATE_AGGREGATE_KEYS)[number]
...
const SUPPORTED_DIMENSION_DATE_AGGREGATE_FUNCS = new Set(ALL_DIMENSION_DATE_AGGREGATE_ITEM_DEFS.map((item) => item.key))
...
export const getDimensionDateAggregateText = (
  ...
  return t(`aggregatesDate${toTranslationKeySuffix(key)}${toTranslationKeySuffix(mode)}`)
}

export const getDimensionDateAggregateItems = (t: Translate): DimensionDateAggregateItem[] => {
...
export const formatDimensionDateAggregate = (aggregate: DimensionDateAggregate | undefined, t: Translate) => {
  if (!aggregate) {
    return undefined
  }
