...
import { getFilterOperatorDisplayLabel, getFilterOperatorOptionLabel } from '../../vbi-chart-where/helpers/where-utils'
...
const DSL_TO_UI_OPERATOR = Object.fromEntries(
  Object.entries(UI_TO_DSL_OPERATOR).map(([uiOperator, dslOperator]) => [dslOperator, uiOperator]),
...
const toHavingAggregateOptions = (items: MeasureAggregateItem[]): HavingAggregateOption[] => {
...
export function getDefaultHavingAggregateByFieldRole(fieldRole: HavingFieldRole): VBIHavingAggregate {
...
export function normalizeHavingAggregate(
  ...
  if (!isAggregateSupportedByFieldRole(aggregate as MeasureAggregate, fieldRole)) {
    return fallback
  }
  ...
  if (aggregate.func === 'quantile') {
    return {
      func: 'quantile',
      quantile: typeof aggregate.quantile === 'number' ? aggregate.quantile : 0.5,
    }
  }
...
export function isHavingNumericAggregate(fieldRole: HavingFieldRole, aggregate: VBIHavingAggregate): boolean {
...
export function getHavingOperatorOptions(isNumericValue: boolean, t: Translate) {
...
export function getHavingAggregateLabel(
  ...
  if (aggregate.func === 'quantile' && mode === 'short') {
    const quantile = 'quantile' in aggregate && typeof aggregate.quantile === 'number' ? aggregate.quantile : 0.5
  ...
  return getMeasureAggregateText(aggregate.func as MeasureAggregateKey, t, mode)
}

export function getHavingDisplayText(item: HavingFilterLike, t: Translate, role: HavingFieldRole = 'measure'): string {
  const normalizedOperator = normalizeHavingOperator(item.operator || item.op)
  const aggregate = normalizeHavingAggregate(item.aggregate, role)
  const aggregateExpr = `${getHavingAggregateLabel(aggregate, t, 'short')}(${item.field})`
