...
| `@visactor/vbi-react/components` submodule                        | **In Progress** | Starter submodule now exports `FieldPanel`, `ChartTypeSelector`, `ChartRenderer`, `BuilderLayout`, `FilterPanel`, and `ThemeSelector`; all stateful components stay hook-based, `FieldPanel` keeps add controls fixed with compact defaults for narrow sidebars while selected lists scroll independently, `FilterPanel` covers first-pass where/having editing with top-level add/edit/remove plus visible nested groups, and richer renderer integration is still pending |
...
| Verification (`typecheck`, tests, build, lint`)                   | **In Progress** | Under Node `24.12.0`, `@visactor/vbi-react` tests and `typecheck` pass with `ThemeSelector` and `FilterPanel` included; package build/lint and broader repo verification are tracked separately                                                                                                                                                                                                                                                                             |
...
- **Completed**: `FieldPanel`, `ChartTypeSelector`, `ChartRenderer`, `BuilderLayout`, `FilterPanel`, `ThemeSelector`
