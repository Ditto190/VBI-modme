...
export const useDimensionActions = () => {
  ...
  const handleDropOnBar = useCallback(
    (e: React.DragEvent, currentGroups: GroupedFields, currentStandalone: FieldItem[]) => {
      e.preventDefault()
      try {
  ...
  const moveToGroup = useCallback(
    (data: DragData, targetCategory: string, currentGroups: GroupedFields, currentStandalone: FieldItem[]) => {
      const nextGroups = { ...currentGroups }
      const nextStandalone = [...currentStandalone]
      const item = extractItem(data.source, data.name, data.groupName, nextGroups, nextStandalone)
  ...
  const updateDimensionAlias = useCallback((name: string, newAlias: string, item: any) => {
    ...
    setStandalone((prev) => prev.map((item) => (item.name === name ? { ...item, alias: newAlias } : item)))

    // 3. 更新本地分组区 (Groups) 状态
    setGroups((prev) => {
      const next = { ...prev }
      Object.keys(next).forEach((key) => {
        next[key] = next[key].map((item) => (item.name === name ? { ...item, alias: newAlias } : item))
      })
      return next
    })
  }, [])
