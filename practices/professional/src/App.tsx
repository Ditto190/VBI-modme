...
import type { VBIChartBuilder, VBIDimension, VBIHavingAggregate, VBIMeasure } from '@visactor/vbi'
import { isVBIHavingFilter } from '@visactor/vbi'
import './App.css'
import { Button, ConfigProvider, Dropdown, Empty, InputNumber, Segmented, Spin, theme, Tooltip } from 'antd'
...
import EncodingPanel, { type MeasureEncodingInfo } from './components/Fields/EncodingPanel'
...
import { PROFESSIONAL_DEFAULT_LIMIT, type ProfessionalLocale, type ProfessionalTheme } from './constants/builder'
...
export function APP() {
  ...
  const { schemaFields, fieldRoleMap, fieldTypeMap } = useVBISchemaFields(builder, schemaRefreshKey)
  const { dimensions, addDimension, removeDimension, updateDimension } = useVBIDimensions(builder)
  const {
  ...
  const availableDimensions = useMemo(() => {
    return schemaFields.filter((field) => field.role === 'dimension').map((field) => field.name)
  }, [schemaFields])

  const availableMeasures = useMemo(() => {
    return schemaFields.filter((field) => field.role === 'measure').map((field) => field.name)
  }, [schemaFields])
  ...
  const dimensionMeasureFields = useMemo(() => {
    return measures.filter((item) => fieldRoleMap[item.field] === 'dimension').map((item) => item.field)
  ...
  const encodingInfo = useMemo<MeasureEncodingInfo[]>(() => {
    ...
    return Array.from(grouped.entries()).map(([encoding, assignedMeasures]) => ({
      encoding,
  ...
  const supportedEncodings = (builder?.chartType.getSupportedMeasureEncodings() ?? []) as MeasureEncoding[]
  const supportedDimensionEncodings = (builder?.chartType.getSupportedDimensionEncodings() ?? []) as DimensionEncoding[]
  ...
  const handleChangeDimensionEncoding = (id: string, encoding: DimensionEncoding) => {
    updateDimension(id, (node) => {
      node.setEncoding(encoding)
    })
  }

  const handleChangeDimensionAggregate = (id: string, aggregate?: DimensionAggregate) => {
    updateDimension(id, (node) => {
      if (aggregate) {
  ...
  const handleChangeAggregateFunc = (field: string, func: MeasureAggregate['func'], quantile?: number) => {
    const target = findMeasureByField(field)
    if (!target) {
      return
    }
    ...
    updateMeasure(target.id, (node) => {
      const aggregate: MeasureAggregate = func === 'quantile' ? { func, quantile } : { func }
      node.setAggregate(aggregate)
    })
  }

  const handleDropMeasureToEncoding = (field: string, encoding: MeasureEncoding) => {
    const target = findMeasureByField(field)
    if (!target) {
  ...
  const handleDropDimensionToEncoding = (field: string, encoding: MeasureEncoding) => {
    const target = findMeasureByField(field)
    if (!target) {
      addMeasure(field, (node) => {
        node.setAlias(field)
  ...
  const handleLoadDemo = async () => {
    try {
      ...
      const schemaByName = new Map(supermarketSchema.map((field) => [field.name, field.type]))
      ...
      const data = dataRows
        .map((values: string[]) => {
          ...
          headers.forEach((header: string, index: number) => {
            ...
            row[header] = schemaByName.get(header) === 'number' ? (rawValue === '' ? null : Number(rawValue)) : rawValue
          })
          return row
        })
