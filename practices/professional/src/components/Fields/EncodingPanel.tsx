...
export interface EncodingPanelProps {
  ...
  onDropDimensionToEncoding?: (field: string, encoding: MeasureEncoding) => void
  title?: string
  emptyText?: string
  dropText?: string
...
const EncodingPanel: React.FC<EncodingPanelProps> = ({
  ...
  const [hoveredEncoding, setHoveredEncoding] = useState<MeasureEncoding | null>(null)
  ...
  const encodingState = useMemo(() => {
    ...
    const state: Partial<Record<MeasureEncoding, { configured: boolean; measures: string[] }>> = {}
  ...
  return (
    <div style={style}>
      ...
      >
        {(Object.entries(encodingState) as Array<[MeasureEncoding, { configured: boolean; measures: string[] }]>).map(
          ([encoding, { configured, measures }]) => (
            <div
              ...
              style={{
                ...
                gap: '8px',
                padding: '6px 12px',
                backgroundColor: hoveredEncoding === encoding ? '#e6f4ff' : configured ? '#fafafa' : '#f5f5f5',
                opacity: configured ? 1 : 0.7,
                borderRadius: '2px',
                fontSize: '12px',
                border: hoveredEncoding === encoding ? '1px dashed #1677ff' : '1px solid transparent',
                transition: 'all 0.12s ease',
              }}
            >
              <span style={{ fontWeight: 'bold', minWidth: '60px', color: '#666' }}>{encoding}</span>
              <div
                style={{
                  flex: 1,
                  display: 'flex',
              ...
              >
                {measures.length > 0 ? (
                  ...
                  <span style={{ color: '#ccc', fontSize: '11px' }}>{dropText}</span>
                )}
              </div>
            </div>
          ),
