...
import { FilterOutlined, DeleteOutlined, PlusOutlined, EditOutlined } from '@ant-design/icons'
...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const availableOperators = React.useMemo(() => {
    return selectedRole === 'measure' ? getMeasureOperators(isZh) : getDimensionOperators(isZh)
  }, [isZh, selectedRole])
  ...
  const handleSubmit = () => {
    form.validateFields().then((values) => {
      ...
      const finalValue =
        (operator === 'in' || operator === 'not in' || operator === '=' || operator === '!=') &&
        typeof value === 'string'
            ...
            <Form.Item label={isZh ? '范围设置' : 'Range'}>
              ...
              >
                <Form.Item name={['value', 'min']} noStyle>
                  <InputNumber placeholder={isZh ? '最小值' : 'Min'} style={{ width: '80px' }} controls={false} />
                </Form.Item>
                ...
                <Form.Item name={['value', 'max']} noStyle>
                  <InputNumber placeholder={isZh ? '最大值' : 'Max'} style={{ width: '80px' }} controls={false} />
                </Form.Item>
              </div>
            </Form.Item>
          ) : (
