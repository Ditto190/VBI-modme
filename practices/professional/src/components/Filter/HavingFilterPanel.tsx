...
import { Button, Card, Form, InputNumber, List, Modal, Select, Space, Tooltip, Typography } from 'antd'
...
export const HavingFilterPanel: React.FC<HavingFilterPanelProps> = ({
  ...
  const aggregateOptions = useMemo(() => {
    return selectedRole === 'dimension' ? DIMENSION_AGGREGATE_OPTIONS : ALL_AGGREGATE_OPTIONS
  }, [selectedRole])
  ...
  const handleEdit = (index: number) => {
    ...
    form.setFieldsValue({
      ...
      range:
        item.operator === 'between' && typeof item.value === 'object' && item.value !== null ? item.value : undefined,
            ...
            <Form.Item label={isZh ? '范围' : 'Range'} required>
              <div style={{ display: 'flex', gap: 8 }}>
                <Form.Item name={['range', 'min']} noStyle rules={[{ required: true, message: '' }]}>
                  <InputNumber style={{ width: '50%' }} placeholder={isZh ? '最小值' : 'Min'} />
                </Form.Item>
                <Form.Item name={['range', 'max']} noStyle rules={[{ required: true, message: '' }]}>
                  <InputNumber style={{ width: '50%' }} placeholder={isZh ? '最大值' : 'Max'} />
                </Form.Item>
              </div>
            </Form.Item>
          ) : (
