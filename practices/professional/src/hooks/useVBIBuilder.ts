...
export const useVBIBuilder = (builder: VBIChartBuilder | undefined) => {
  ...
  const setLocale = useCallback((locale: ProfessionalLocale) => builder?.locale.setLocale(locale), [builder])
  const setTheme = useCallback((theme: ProfessionalTheme) => builder?.theme.setTheme(theme), [builder])
  const setLimit = useCallback((limit: number) => builder?.limit.setLimit(normalizeLimit(limit)), [builder])
