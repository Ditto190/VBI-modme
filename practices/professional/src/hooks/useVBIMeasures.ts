...
import { type VBIChartBuilder, type VBIMeasure as CoreVBIMeasure } from '@visactor/vbi'
...
export const useVBIMeasures = (builder: VBIChartBuilder | undefined) => {
  const measures = useBuilderDocState({
    ...
    getSnapshot: (activeBuilder) => activeBuilder.measures.toJSON() as VBIMeasure[],
  })
  ...
  const removeMeasure = useCallback((id: string) => builder?.doc.transact(() => builder.measures.remove(id)), [builder])
