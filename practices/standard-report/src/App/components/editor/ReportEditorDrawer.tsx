...
const createSafeSourceRect = (pageId?: string, sourceRect?: EditorSourceRect | null): EditorSourceRect | null => {
  if (sourceRect) {
    return sourceRect
  }
...
export const ReportEditorDrawer = () => {
  ...
  const [frameRect, setFrameRect] = useState<EditorSourceRect>(() =>
    typeof window === 'undefined' ? { top: 0, left: 0, width: 0, height: 0, borderRadius: 0 } : getViewportRect(),
  )
  ...
  const beginClose = useCallback(() => {
    ...
    const targetRect =
      createSafeSourceRect(pageId, readEditorSourceRect(findPreviewElement(pageId))) ?? sourceRect ?? getFallbackRect()
  ...
  useEffect(() => {
    ...
    const nextSourceRect = createSafeSourceRect(pageId, sourceRect) ?? getFallbackRect()
