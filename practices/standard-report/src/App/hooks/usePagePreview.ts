...
const getInitialState = (reportBuilder: VBIReportBuilder, pageId: string): PreviewState => {
...
export const usePagePreview = (reportBuilder: VBIReportBuilder, pageId: string, revision: number) => {
  const [state, setState] = useState<PreviewState>(() => getInitialState(reportBuilder, pageId))
