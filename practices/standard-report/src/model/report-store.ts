...
import { addReportPage, ensureReportHasPage, getFallbackActivePageId } from 'src/utils/report-pages'
...
export interface ReportStoreState {
  ...
  initialize: (reportBuilder?: VBIReportBuilder, vbi?: typeof VBI) => DestroyCallback
...
export const useReportStore = create<ReportStoreState>((set, get) => ({
  ...
  initialize: (reportBuilder, vbi) => {
    ...
    const activePageId = ensureReportHasPage(nextReportBuilder, connectorId, nextVBI)
    set({
  ...
  removePage: (pageId) => {
    ...
    const activePageId = getFallbackActivePageId(pages, pageId, get().activePageId)
    get().reportBuilder.page.remove(pageId)
    const ensuredPageId =
      get().reportBuilder.build().pages[0]?.id ?? addReportPage(get().reportBuilder, connectorId, { vbi: get().vbi })
