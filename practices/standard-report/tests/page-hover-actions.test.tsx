...
test('renders edit, add and remove actions in the shared toolbar', () => {
  ...
  const toolbar = within(view.container).getByTestId('standard-report-actions')
  expect(within(toolbar).getByRole('button', { name: '编辑当前图表' })).toBeInTheDocument()
  expect(within(toolbar).getByRole('button', { name: '新增一页' })).toBeInTheDocument()
  expect(within(toolbar).getByRole('button', { name: '删除当前页' })).toBeInTheDocument()
})
