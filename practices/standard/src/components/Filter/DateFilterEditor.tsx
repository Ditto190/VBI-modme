...
export const DateFilterEditor: React.FC<DateFilterEditorProps> = ({ value, onChange }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {predicate.type === 'range' && <RangeFields value={predicate} onChange={onChange} />}
      {predicate.type === 'relative' && <RelativeFields value={predicate} onChange={onChange} />}
      {predicate.type === 'current' && <CurrentFields value={predicate} onChange={onChange} />}
      {predicate.type === 'period' && <PeriodFields value={predicate} onChange={onChange} />}
    </div>
  )
}
...
function getPeriodPickerFormat(unit: PeriodValue['unit']): DatePickerProps['format'] {
  switch (unit) {
    case 'year':
      return 'YYYY'
    case 'quarter':
...
function getPeriodPlaceholder(unit: PeriodValue['unit'], t: (key: string) => string): string {
  switch (unit) {
    case 'year':
