...
export const createFieldDragPayload = (params: { field: string; type?: string; role: FieldRole }): FieldDragPayload => {
  return {
    kind: 'field',
    field: params.field,
    type: params.type,
...
export const writeFieldDragPayload = (e: React.DragEvent, payload: FieldDragPayload) => {
  e.dataTransfer.setData('application/json', JSON.stringify(payload))
  e.dataTransfer.effectAllowed = 'copy'
}

export const readFieldDragPayload = (e: React.DragEvent): FieldDragPayload | undefined => {
...
export const readShelfDragIndex = (e: React.DragEvent): number | undefined => {
  ...
  if (rawValue.startsWith(SHELF_INDEX_PREFIX)) {
    const parsed = Number.parseInt(rawValue.slice(SHELF_INDEX_PREFIX.length), 10)
