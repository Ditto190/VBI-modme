...
import { isDateDimensionField, normalizeDimensionDateAggregate } from './dimensionDateAggregateUtils'
...
export const getMeasureMenuSelectedKeys = (measure: {
  ...
  keys.push(`aggregate:quantile:${toQuantilePercent(measure.aggregate.quantile)}`)
  return keys
}
...
export const getDimensionMenuSelectedKeys = (
  ...
  const aggregate = normalizeDimensionDateAggregate(dimension.aggregate, schemaType)
