...
import { VBIChartBuilder, type VBIDimension as CoreVBIDimension, type VBISort } from '@visactor/vbi'
 ...
 * 提供维度管理
 */
export const useVBIDimensions = (builder: VBIChartBuilder | undefined): UseVBIDimensionsResult => {
  const dimensions = useBuilderDocState({
    builder,
    fallback: EMPTY_DIMENSIONS,
    getSnapshot: (activeBuilder) => activeBuilder.dimensions.toJSON() as VBIDimension[],
  })
