...
type UseVBIHavingFilterResult = {
  ...
  addGroup: (op: 'and' | 'or', callback?: (group: HavingGroupLike) => void) => void
 ...
 * 支持响应式同步和增量操作
 */
export const useVBIHavingFilter = (builder: VBIChartBuilder | undefined): UseVBIHavingFilterResult => {
  const filters = useBuilderDocState({
    builder,
    fallback: EMPTY_HAVING_CLAUSES,
    getSnapshot: (activeBuilder) => activeBuilder.havingFilter.toJSON().conditions as VBIHavingClause[],
  })

  const addFilter = useCallback(
    (field: string, aggregate?: VBIHavingAggregate, operator?: string, value?: unknown) => {
      if (!builder) {
        return
      }
