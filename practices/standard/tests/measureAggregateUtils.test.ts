...
test('checks aggregate support by field role', () => {
  ...
  expect(isAggregateSupportedByFieldRole({ func: 'min' }, 'dimension')).toBe(true)
  expect(isAggregateSupportedByFieldRole({ func: 'sum' }, 'dimension')).toBe(false)
  expect(isAggregateSupportedByFieldRole({ func: 'quantile', quantile: 0.8 }, 'dimension')).toBe(false)
})
